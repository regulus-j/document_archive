...
<div class="bg-gray-100 min-h-screen py-6 sm:py-12">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <!-- Default View -->
        <div id="defaultView" class="bg-white shadow rounded-lg overflow-hidden">
            <div class="overflow-x-auto">
                <table id="officesTable" class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('Office Name') }}</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="office-row hover:bg-gray-50">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-gray-900 office-name">{{ $office->name }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-500 office-created">{{ $office->created_at->format('M d, Y') }}</div>
                            </td>
        ...
        <!-- Hierarchical View -->
        <div id="hierarchicalView" class="bg-white shadow rounded-lg overflow-hidden hidden">
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('Name') }}</th>
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{{ __('Offices') }}</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-gray-200">
                        @php
...
$childOfficeGroups = $officesByParent->except(null);
                        ...
                        @foreach ($topLevelOffices as $office)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                    <a href="{{ route('office.show', $office->id) }}" class="hover:text-blue-600">{{ $office->name }}</a>
                                </td>
                                    ...
                                    @if ($childOffices && $childOffices->count() > 0)
                                        <ul class="list-disc pl-5">
                                            @foreach ($childOffices as $childOffice)
                                                <li><a href="{{ route('office.show', $childOffice->id) }}" class="hover:text-blue-600">{{ $childOffice->name }}</a></li>
                                            @endforeach
                                        </ul>
                                    @else
                                        {{ __('N/A') }}
    ...
    $parentOffice = $offices->firstWhere('id', $parentId);
                            ...
                            @if ($parentOffice && !$topLevelOffices->contains($parentOffice))
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                        <a href="{{ route('office.show', $parentOffice->id) }}" class="hover:text-blue-600">{{ $parentOffice->name }}</a>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        <ul class="list-disc pl-5">
                                            @foreach ($childOffices as $childOffice)
