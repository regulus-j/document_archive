...
<head>
    ...
    <title>{{ config('app.name', 'DocArchive') }} - Document Archiving System</title>
                ...
                <ul class="space-y-1">
                    ...
                    <li x-data="{ open: false }">
                        <button @click="open = !open" 
                                class="flex items-center w-full px-4 py-2 text-gray-600 hover:bg-blue-50 hover:text-blue-600">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
                        ...
                        <div x-show="open" 
                             ...
                             class="pl-11 pr-4 py-2 space-y-1">
                            <a href="{{ route('documents.index') }}" 
                               class="block py-2 text-sm text-gray-600 hover:text-blue-600">{{ __('View') }}</a>
                            <a href="{{ route('documents.archive') }}" 
                               class="block py-2 text-sm text-gray-600 hover:text-blue-600">{{ __('Archives') }}</a>
                            <a href="{{ route('documents.create') }}" 
                               class="block py-2 text-sm text-gray-600 hover:text-blue-600">{{ __('Upload') }}</a>
                            <a href="{{ route('documents.workflows') }}" 
                               class="block py-2 text-sm text-gray-600 hover:text-blue-600">{{ __('Received') }}</a>
                        </div>
                    </li>
                    @endcan
            ...
            <div class="border-t border-gray-200 p-4">
                ...
                <div class="mt-3 space-y-1">
                    <a href="{{ route('profile.edit') }}" 
                       class="block px-4 py-2 text-gray-600 hover:bg-blue-50 hover:text-blue-600">{{ __('Profile') }}</a>
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <a href="{{ route('logout') }}" 
                           onclick="event.preventDefault(); this.closest('form').submit();" 
                           class="block px-4 py-2 text-gray-600 hover:bg-blue-50 hover:text-blue-600">{{ __('Log Out') }}</a>
                    </form>
                </div>
            </div>
        </aside>
