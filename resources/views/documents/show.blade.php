        ...
        <div class="bg-white rounded-xl shadow-xl overflow-hidden mb-8 border border-blue-100">
            <div class="p-6 border-b border-gray-200">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                    ...
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800 mt-2 md:mt-0">
            ...
            <div class="p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                    <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
                        ...
                        <p class="text-base font-medium text-gray-900">{{ $document->trackingNumber->tracking_number ?? 'N/A' }}</p>
                    </div>

                    <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
                        <p class="text-sm font-medium text-gray-500 mb-1">Classification</p>
                        <p class="text-base font-medium text-gray-900">{{ $document->categories->first()->category ?? 'N/A' }}</p>
                    </div>

                    <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
                        <p class="text-sm font-medium text-gray-500 mb-1">From Office</p>
                        <p class="text-base font-medium text-gray-900">{{ $document->transaction->fromOffice->name ?? 'N/A' }}</p>
                    </div>

                    <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
                        <p class="text-sm font-medium text-gray-500 mb-1">To Office</p>
                        <p class="text-base font-medium text-gray-900">{{ $document->transaction->toOffice->name ?? 'N/A' }}</p>
                    </div>
                    ...
                    <div class="overflow-x-auto rounded-lg border border-gray-200">
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-blue-700 uppercase tracking-wider">Receive Order</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-blue-700 uppercase tracking-wider">Recipient</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                ...
                                <tr class="hover:bg-gray-50 transition-colors">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $workflow->step_order }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $workflow->recipient->first_name }} {{ $workflow->recipient->last_name }}</td>
                            ...
                            <ul class="space-y-2 pl-6">
                                ...
                                <li>
                                    <a href="{{ route('documents.download', $attachment->id) }}"
                                        ...
                                        <svg class="w-4 h-4 mr-2 text-gray-500 group-hover:text-blue-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" />
                                        </svg>
                                        {{ $attachment->filename }}
                                    </a>
        ...
        <div class="bg-white rounded-xl shadow-xl overflow-hidden border border-blue-100 mb-8">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-medium text-blue-700 uppercase tracking-wider">Date/Time</th>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-blue-700 uppercase tracking-wider">Status</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-blue-700 uppercase tracking-wider">Details</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($auditLogs as $log)
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $log->created_at->format('M d, Y H:i:s') }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $log->user->first_name }} {{ $log->user->last_name }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $log->status }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ Str::limit($log->details, 50) }}</td>
                            </tr>
                        @empty
                            <tr>
